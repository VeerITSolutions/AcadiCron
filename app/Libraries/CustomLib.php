...
class CustomLib
{
...

    {
        ...
        $setting_result = $this->settingModel->getSchoolDetail();
    ...
    {
        // Fetch the settings using the model
        $setting_result = $this->settingModel->get();

        if (!empty($setting_result) && isset($setting_result[0]['date_format'])) {
            return $setting_result[0]['date_format'];
        }

        // Return a default value if settings are not found
        return 'Y-m-d'; // Example default date format
    }
...

    {
        ...
        $result=$this->settingModel->get(1);
    ...
    public function getLoggedInUserData()
{
    ...
    // Optionally return null or an empty array if neither session is found
    return null;
}
...

    {
        ...
        if ($user_role == "parent") {
            ...
            $get_student_class_id = $student['class_id'];

            $this->db->select('*');
            $this->db->from('send_notification');
            $this->db->where('class_id', $get_student_class_id);*/
    ...
    {
        // Get the logged-in user data (assumes getLoggedInUserData() is implemented with session or Auth)
        ...
        // Assuming 'Staff' is the corresponding model for the staff table
        $data = \App\Models\Staff::find($id);  // Fetch staff data by ID

        // Get school settings
        $setting_result = SchSettings::first();  // Assuming you have a Setting model

        if ($setting_result) {
            $data['class_teacher'] = $setting_result->class_teacher;  // Add class_teacher from settings
        } else {
            $data['class_teacher'] = 'yes';  // Default value if no setting is found
        }
 ...
 public function get_betweendate($search_type)
...

    {
        ...
        $result = $this->settingModel->getLanguage();
...

    {
        ...
        $result = $this->settingModel->getuserLanguage();
